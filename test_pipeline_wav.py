...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--wav",
        default="backend/recordings/testingdata.wav",
        help="Path to WAV file to stream",
    )
    parser.add_argument("--chunk-ms", type=int, default=250, help="Chunk duration in milliseconds")
    parser.add_argument("--max-chunks", type=int, default=40, help="Maximum chunks to stream")
    parser.add_argument("--stop-timeout", type=float, default=30.0, help="Seconds to wait for session_complete")
...
async def main() -> None:
    ...
    async with websockets.connect(ws_url, max_size=10_000_000) as ws:
        for chunk_index, offset in enumerate(range(0, len(raw_audio), bytes_per_chunk)):
        ...
        completion = json.loads(await asyncio.wait_for(ws.recv(), timeout=args.stop_timeout))
