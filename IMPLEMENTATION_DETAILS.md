...
[!NOTE]
## Sequence Diagram: Audio Processing Pipeline
...
sequenceDiagram
  ...
  participant DB
  Client->>FastAPI: POST /sessions (start session)
  FastAPI->>DB: Create session row
  FastAPI-->>Client: session_id
  Client->>WebSocket: Connect /ws/{session_id}
  loop For each audio chunk
    Client->>WebSocket: Send audio chunk (bytes)
    WebSocket->>Orchestrator: Process chunk
    Orchestrator->>Services: Analyze (loudness, diarization, etc.)
    Services-->>Orchestrator: Results (speaker, transcript, etc.)
    Orchestrator->>DB: Store insight
    Orchestrator-->>WebSocket: Emit insight JSON
    WebSocket-->>Client: Send insight JSON
  end
  Client->>WebSocket: Send 'session_complete'
  WebSocket->>Orchestrator: Finalize session
  Orchestrator->>DB: Aggregate speakers, finalize
  Orchestrator-->>WebSocket: Emit session_complete JSON
  WebSocket-->>Client: Send session_complete JSON
  Client->>FastAPI: GET /sessions/{id}/insights
  FastAPI->>DB: Query insights
  FastAPI-->>Client: Return insights JSON
```
